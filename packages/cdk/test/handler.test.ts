...
async function gated(
  ...
  if (event.RequestType === "Delete") return { PhysicalResourceId: physicalId, Data: { skipped: "delete" } };
...
describe("runnerHandler.gated", () => {
  ...
  it("short-circuits on run mode 'off'", async () => {
    let called = 0;
    const res = await gated({ RequestType: "Create" }, { AM_STATE_TABLE: "t", AM_RUN_MODE: "off" }, async () => {
      called++;
      return 1;
    });
  ...
  it("throws without AM_STATE_TABLE", async () => {
    await assert.rejects(() => gated({ RequestType: "Create" }, {}, async () => 0), /AM_STATE_TABLE/);
  });
});
