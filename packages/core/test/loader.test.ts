...
describe('loader', () => {
  ...
  it('changes checksum when file contents change', async () => {
    ...
    const [beforeMig] = await discoverMigrations(dir);
    assert.ok(beforeMig, 'expected at least one migration');
    const before = beforeMig.checksum;
    await writeFile(file, 'v2');
    const [afterMig] = await discoverMigrations(dir);
    assert.ok(afterMig, 'expected at least one migration');
    const after = afterMig.checksum;
